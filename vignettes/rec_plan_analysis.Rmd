...
```{r prop_no_plan, echo = FALSE, warning=FALSE}
n_listed <- unlist(lapply(years, 
...
n_list_df <- data.frame(years, n_listed, n_plans)
n_list_df$pct_w_plan <- n_list_df$n_plans / n_list_df$n_listed

gg <- ggplot(n_list_df, aes(y = pct_w_plan, x = years)) +
        geom_point(size = 3, alpha = 0.5) +
        geom_smooth(level = 0.9, alpha = 0, colour = "black") +
        scale_y_continuous(limits = c(0,1)) +
        ggtitle("Proportion of species with recovery plans", 
                subtitle = "By listing year") +
        labs(x = "",
             y = "Count") +
        theme_hc() + 
        theme(text = element_text(family = "Open Sans"))
