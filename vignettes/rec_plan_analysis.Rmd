...
no_plan <- filter(listed, is.na(listed$Year_Plan) | listed$Plan_Status != "Final")
...
ggplot(n_list_df, aes(y = n_listed, x = years)) +
  ...
  geom_line(aes(y = n_plans, x = years), 
            colour = "gray70",
            linetype = "dashed") +
  ggtitle("Number of species without final recovery plans (black)", 
...
This pattern highlights the challenge of interpreting the time-to-plan figures because of the right censored data. The `r n_list_df$n_no_plan[length(n_list_df$n_no_plan)]` species without plans effectively have a time-to-plan of infinity. 
